...
# # file_path = "C:/Users/louay/Desktop/captured_image2.jpg"
...
# cap_builtin = cv2.VideoCapture(0,cv2.CAP_DSHOW)

# # Open the first external camera
# cap_external1 = cv2.VideoCapture(1,cv2.CAP_DSHOW)

# # Open the second external camera
# cap_external2 = cv2.VideoCapture(2,cv2.CAP_DSHOW)

# while True:
#     # Capture frames from cameras
#     ret_builtin, frame_builtin = cap_builtin.read()
#     ret_external1, frame_external1 = cap_external1.read()
#     ret_external2, frame_external2 = cap_external2.read()

#     # Display frames from cameras
#     cv2.imshow('Built-in Camera', frame_builtin)
#     cv2.imshow('External Camera 1', frame_external1)
#     cv2.imshow('External Camera 2', frame_external2)
...
# cap_builtin.release()
# cap_external1.release()
# cap_external2.release()
...
#    adding logo (remember image should be PNG and not JPG)
    ...
    def show_frames():
        ...
        if nb_camera_stack == 2 and nb_camera_extra == 0:
            ...
            frame0 = frame0[offset_Ly+50:frame0.shape[0] -
                            offset_Ry-500, 230:frame0.shape[1]-offset_Lx]
            ...
            frame1 = frame1[offset_Ry+50:frame1.shape[0] -
                            offset_Ly-500, offset_Rx:frame1.shape[1]-250]
            ...
            ### CONTOUR ###
            ###############
            gray_img = cv2.cvtColor(cv2image0, cv2.COLOR_BGR2GRAY)
    ...
    def take_pic():
        ...
        if zoom_done == 'yes' and (lookup.iloc[0]['cam1']) == 'yes':
            ...
            w = imgtk2.width()
            h = imgtk2.height()
            merged_image = Image.new("RGB", (w+4, h + 230), color="white")
            ...
            merged_image.paste(ImageTk.getimage(imgtk_zoomed2), (150, h+40))
            merged_image.paste(ImageTk.getimage(imgtk_zoomed3), (300, h+40))
            merged_image.paste(ImageTk.getimage(imgtk_zoomed4), (450, h+40))
            merged_image.paste(ImageTk.getimage(imgtk_zoomed5), (600, h+40))
            ...
            draw.text((180, h + 210), date_string, (0, 0, 0), font)
    ...
    def Analyze():
        ...
        if (lookup.iloc[0]['cam1']) == 'yes':
            ...
            label_workspace.place(x=510, y=268)
            # Area for zoom1
...
# provisoire !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            imgtk_zoomed1 = zoom(img2, 300, 100)
            imgtk_zoomed2 = zoom(img2, 200, 100)
            imgtk_zoomed3 = zoom(img2, 150, 150)
            imgtk_zoomed4 = zoom(img2, 200, 200)
